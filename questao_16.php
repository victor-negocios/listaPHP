<?php
// Questão 16: Qual a média dos valores das transações realizadas por pessoas do gênero 'feminino'?
...
foreach($transacoes as $transacao){
  if($transacao['genero']== "feminino" && $transacao['tipoDeTransacao']== "Despesa"){
    $qtdDespesas += 1;
    $mediaDespesas += $transacao['valor'];
  }else if($transacao['genero'] == "feminino" && $transacao['tipoDeTransacao'] == "Receita"){
    $qtdReceitas += 1;
    $mediaReceitas += $transacao['valor'];
  }
}


if($mediaDespesas <> 0 && $mediaReceitas <> 0){
  $mediaDespesas = $mediaDespesas/$qtdDespesas;
  $mediaReceitas = $mediaReceitas/$qtdReceitas;

  echo "Média Despesas Femininas: $mediaDespesas <br>";
  echo "Média Receitas Femininas: $mediaReceitas";
}
else if ($mediaDespesas == 0){
  $mediaReceitas = $mediaReceitas/$qtdReceitas;
